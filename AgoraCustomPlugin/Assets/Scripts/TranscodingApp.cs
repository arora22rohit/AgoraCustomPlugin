...
public class TranscodingApp : PlayerViewControllerBase
{
    ...
    protected void StartSharing()
    {
        if (running == false)
        {
            ...
            mRect = new Rect(0, 0, resolution.x, resolution.y);
            // Create a texture the size of the rectangle you just created
            mTexture = new Texture2D(resolution.x, resolution.y, TextureFormat.RGBA32, false);
            //mTexture.Resize(1280,720);
            mTexture.Apply();
    ...
    IEnumerator shareScreen()
    {
        while (running)
        {
            ...
            //mTexture.ReadPixels(new Rect(0, 0, resolution.x-2, resolution.y-2), 0, 0, false);
            mTexture.Apply();
            TextureScale.Bilinear(mTexture, resolution.x, resolution.y);
            Debug.LogError("Current Resolution: " + mTexture.width+ " H: "+ mTexture.height);
            ...
            if (mRtcEngine != null)
            {
                ...
                externalVideoFrame.stride = (int)resolution.x;
    ...
    void StartTranscoding(uint uid)
    {
        ...
        user.x = 0;
        user.y = 0;
        user.width = Screen.width;
        user.height = Screen.height;
        user.audioChannel = 0;
        user.alpha = 1;
